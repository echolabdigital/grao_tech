...
function ProofStrip() {
  ...
  return (
    <div style={{ background:B.cream2, borderTop:`1px solid ${B.latte}`,
      borderBottom:`1px solid ${B.latte}`, padding:".85rem 1.5rem" }}>
      <div style={{ maxWidth:1180, margin:"0 auto", display:"flex",
        alignItems:"center", gap:"2.5rem", flexWrap:"wrap", justifyContent:"center" }}>
        <span style={{ fontFamily:"'Trebuchet MS',sans-serif", fontSize:".58rem",
          letterSpacing:".18em", textTransform:"uppercase", color:B.muted }}>
          Integrado com
        </span>
        {items.map(p => (
          <div key={p.name} style={{ display:"flex", alignItems:"center", gap:".4rem",
            fontFamily:"'Trebuchet MS',sans-serif", fontSize:".75rem",
            fontWeight:600, color:B.brown, opacity:.7 }}>
            <span style={{ fontSize:"1rem" }}>{p.icon}</span>{p.name}
          </div>
        ))}
      </div>
    </div>
...
function HowItWorks() {
  ...
  return (
    <section id="como" style={{ background:B.cream, padding:"100px 1.5rem",
      borderTop:`1px solid ${B.cream2}` }}>
      <div style={{ maxWidth:1180, margin:"0 auto" }}>
        <Fade>
          <span style={{ fontFamily:"'Trebuchet MS',sans-serif", fontSize:".6rem",
            letterSpacing:".2em", textTransform:"uppercase", color:B.terra,
            display:"block", marginBottom:".6rem" }}>Como funciona</span>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))" }}>
          {steps.map((s,i) => (
            <Fade key={s.n} delay={.08*i}>
              <div style={{ padding:"2.4rem 2rem", position:"relative",
                borderLeft:i===0?"none":`1px dashed ${B.sand}` }}>
                {i>0 && (
                  <div style={{ position:"absolute", left:-10, top:"3rem",
                ...
                <div style={{ fontFamily:"Georgia,serif", fontSize:"3.2rem",
                  color:B.cream2, lineHeight:1, marginBottom:".7rem", userSelect:"none" }}>{s.n}</div>
                <div style={{ fontSize:"1.5rem", marginBottom:".6rem" }}>{s.icon}</div>
                <h3 style={{ fontFamily:"Georgia,serif", fontSize:"1.12rem",
                  color:B.ink, fontWeight:700, marginBottom:".6rem" }}>{s.title}</h3>
                <p style={{ fontFamily:"'Trebuchet MS',sans-serif", fontSize:".78rem",
                  color:B.muted, lineHeight:1.75 }}>{s.desc}</p>
              </div>
            </Fade>
          ))}
        </div>
...
function IntegrationsHub() {
  ...
  return (
    <section style={{ background:B.ink, padding:"100px 1.5rem",
      ...
      <div style={{ maxWidth:1180, margin:"0 auto" }}>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr",
          ...
          <Fade>
            <span style={{ fontFamily:"'Trebuchet MS',sans-serif", fontSize:".6rem",
              letterSpacing:".2em", textTransform:"uppercase", color:B.gold,
              display:"block", marginBottom:".6rem" }}>Integrações</span>
            <h2 style={{ fontFamily:"Georgia,serif", fontSize:"clamp(1.8rem,3.5vw,2.8rem)",
              color:B.cream, fontWeight:400, lineHeight:1.15, marginBottom:"1rem" }}>
              Tudo conectado,{" "}
              <em style={{ color:B.wheat }}>tudo no mesmo lugar.</em>
            </h2>
            <p style={{ fontFamily:"'Trebuchet MS',sans-serif", fontSize:".86rem",
            ...
            {[
              ...
              <div key={it.name} style={{ display:"flex", alignItems:"center", gap:".9rem",
                padding:".8rem 1rem", background:"rgba(255,255,255,.04)",
                border:"1px solid rgba(255,255,255,.07)", marginBottom:".5rem",
                transition:".2s", cursor:"default" }}
                onMouseEnter={e=>{e.currentTarget.style.borderColor=it.color+"44";e.currentTarget.style.background="rgba(255,255,255,.07)"}}
                ...
                <div style={{ flex:1 }}>
                  <div style={{ fontFamily:"'Trebuchet MS',sans-serif", fontSize:".8rem",
                    fontWeight:700, color:B.cream }}>{it.name}</div>
                  <div style={{ fontFamily:"'Trebuchet MS',sans-serif", fontSize:".64rem",
                    color:"rgba(250,243,230,.38)" }}>{it.desc}</div>
                </div>
          ...
          <Fade delay={.2}>
            <div style={{ position:"relative", height:440,
              display:"flex", alignItems:"center", justifyContent:"center" }}>
              {[150,200].map((r,ri) => (
                <div key={r} style={{ position:"absolute", width:r*2, height:r*2,
                  borderRadius:"50%", border:`1px dashed rgba(255,255,255,${ri===0?.07:.04})`,
                  animation:`rotateSlow ${20+ri*8}s linear infinite${ri===1?" reverse":""}` }}/>
              ))}
              <div style={{ position:"absolute", zIndex:3, width:90, height:90,
                borderRadius:"50%", background:`linear-gradient(135deg,${B.terra},${B.brown})`,
              ...
              {nodes.map(n => {
                ...
                return (
                  <div key={n.label} style={{ position:"absolute", zIndex:4,
                    left:`calc(50% + ${nx}px - 34px)`, top:`calc(50% + ${ny}px - 34px)`,
                    width:68, height:68, borderRadius:"50%",
                    background:"rgba(0,0,0,.4)", border:`2px solid ${n.color}44`,
                    display:"flex", flexDirection:"column", alignItems:"center",
                    justifyContent:"center", gap:2, transition:".2s", cursor:"default" }}
                    onMouseEnter={e=>{e.currentTarget.style.borderColor=n.color;e.currentTarget.style.background=n.color+"28"}}
                    onMouseLeave={e=>{e.currentTarget.style.borderColor=n.color+"44";e.currentTarget.style.background="rgba(0,0,0,.4)"}}>
                    <span style={{ fontSize:"1.25rem" }}>{n.icon}</span>
                    <span style={{ fontFamily:"'Trebuchet MS',sans-serif", fontSize:".5rem",
                      letterSpacing:".06em", color:"rgba(255,255,255,.45)", textTransform:"uppercase" }}>
                      {n.label}
                    </span>
                  </div>
                );
  ...
  return (
    <section id="clientes" style={{
      ...
      <div style={{ maxWidth:1180, margin:"0 auto" }}>
        <Fade>
          <span style={{ fontFamily:"'Trebuchet MS',sans-serif", fontSize:".6rem",
            letterSpacing:".2em", textTransform:"uppercase", color:B.wheat,
          ...
          {allReviews.filter(r => r.name !== featured.name).map((r,i) => (
            <Fade key={r.name} delay={.1*i}>
              <div style={{ background:"rgba(255,255,255,.07)", padding:"2.2rem",
                ...
                <div style={{ marginBottom:"1rem" }}>
                  {"★★★★★".split("").map((_,j) => (
                    <span key={j} style={{ color:B.gold, fontSize:".9rem" }}>★</span>
                  ))}
                </div>
                <p style={{ fontFamily:"Georgia,serif", fontSize:".95rem",
                  fontStyle:"italic", color:"rgba(250,243,230,.85)",
                  lineHeight:1.75, marginBottom:"1.4rem" }}>"{r.text}"</p>
                <div style={{ display:"flex", alignItems:"center", gap:".8rem" }}>
                  <div style={{ width:40, height:40, borderRadius:"50%",
                    background:B.terra, display:"flex", alignItems:"center",
                    justifyContent:"center", flexShrink:0,
                    fontFamily:"Georgia,serif", fontSize:".95rem", color:"#fff", fontWeight:700 }}>
                    {r.i}
                  </div>
                  <div>
                    <div style={{ fontFamily:"'Trebuchet MS',sans-serif", fontSize:".78rem",
                      fontWeight:700, color:B.cream }}>{r.name}</div>
                    <div style={{ fontFamily:"'Trebuchet MS',sans-serif", fontSize:".63rem",
                      color:"rgba(250,243,230,.4)" }}>{r.role}</div>
                  </div>
                </div>
              </div>
            </Fade>
